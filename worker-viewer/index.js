// PRETASE Main Worker - Auth + Viewer (single domain)
// Login, session, dashboard, sheet viewer
...
function loginPage() {
  return new Response(readTemplate('login.html'), { headers: { 'Content-Type': 'text/html' } });
}
...
function dashboardPage(session) {
  ...
  html = html.replace('__USERNAME__', esc(session.username));
  html = html.replace('__ROLE__', esc(session.role));
  html = html.replace('__CARDS__', cards);
  return new Response(html, { headers: { 'Content-Type': 'text/html' } });
}

function sheetPage(sheetFile) {
  return new Response(SHEET_HTML.replace('__SHEET_FILE__', esc(sheetFile)).replace('__CDN_BASE__', CDN_BASE), { headers: { 'Content-Type': 'text/html' } });
}
